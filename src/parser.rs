use {
  nom::{
    ...
    branch::{
      alt, // takes a tuple of parsers, try up to 21 parsers and return result
      // permutation, // all parsers must success in any order. parsers are matched greedily
    },
  },
...
fn bold(input: &str) -> IResult<&str, ICreole<'_>> {
  map(delimited(tag("**"), take_until("**"), tag("**")), |body| ICreole::Bold(body))
    (input)
}
fn italic(input: &str) -> IResult<&str, ICreole<'_>> {
  map(delimited(tag("//"), take_until("//"), tag("//")), |body| ICreole::Italic(body))
    (input)
}
...
fn list(input: &str) -> IResult<&str, ICreole> {
  ...
  if lines.is_empty() {
    return Err(nom::Err::Error(nom::error::Error{input, code: ErrorKind::Tag}))
  }
...
fn _list<'a>(head_tag: &'a str) -> impl FnMut(Vec<&'a str>) -> Result<ICreole, nom::Err<nom::error::Error<&'a str>>> {
  move |input: Vec<&str>| {
    ...
    for i in 0..input.len() {
      ...
      if line.starts_with(&head_space) { // sibling
        if let Ok((_, v)) = map(alt((
          collect_opt_pair1(take_while_parser_fail(lit, text)),
          success(vec![])
        )), |v| ICreole::ListItem(v))(&line[head_space.len()..]){
          rst.push(v);
        }
      } else { // child
...
fn link(input: &str) -> IResult<&str, ICreole> {
  map(delimited(tag("[["), alt((
    ...
    map(is_not("[]\n"), |v: &str| (v, v))
  )), tag("]]")), |(link, label)| ICreole::Link(link, label))(input)
}
fn image(input: &str) -> IResult<&str, ICreole> {
  map(delimited(tag("{{"), alt((
    separated_pair(is_not("{}|\n"), tag("|"), is_not("{}\n")),
    map(is_not("{}\n"), |src: &str| (src, "")),
  )), tag("}}")), |(src, label)| ICreole::Image(src, label))(input)
}

fn lit(input: &str) -> IResult<&str, ICreole> {
  alt((
    link, image, text_style,
  ))(input)
}
fn text(input: &str) -> IResult<&str, ICreole> {
  map(verify(rest, |s: &str| !s.is_empty()), |s: &str| ICreole::Text(s))(input)
}
fn take_lit_text_until0(until_tag: &'static str) -> impl FnMut(&str) -> IResult<&str, Vec<ICreole>> {
  move |input: &str|
    collect_opt_pair0(take_while_parser_fail_or_peek_tag(until_tag, lit, text))
      (input)
}

fn take_while_parser_fail<'a, F, G>(mut parser :F, mut fail_parser: G) -> impl FnMut(&'a str) -> IResult<&'a str, (Option<ICreole<'a>>, Option<ICreole<'a>>)>
where F: FnMut(&str) -> IResult<&str, ICreole>,
...
fn collect_opt_pair0<'a, F>(mut parser :F) -> impl FnMut(&'a str) -> IResult<&'a str, Vec<ICreole<'a>>>
  where F: FnMut(&'a str) -> IResult<&'a str, (Option<ICreole<'a>>, Option<ICreole<'a>>)>
{
  move |input: &str| {
    let mut rst = vec![];
    let mut i = input;
    ...
    while let Ok((r, t)) = parser(i) {
      ...
      i = r;
      match t {
        (Some(a), Some(b)) => { rst.push(a); rst.push(b); }
        (None, Some(b)) => { rst.push(b); }
        (Some(a), None) => { rst.push(a); break; }
        _ => { break; }
      }
    }
...
fn collect_opt_pair1<'a, F>(mut parser :F) -> impl FnMut(&'a str) -> IResult<&'a str, Vec<ICreole<'a>>>
  where F: FnMut(&'a str) -> IResult<&'a str, (Option<ICreole<'a>>, Option<ICreole<'a>>)>
{
  move |input: &str| {
    let mut rst = vec![];
...
fn take_while_parser_fail_or<'a, F, G, H>(mut term_parser: F, mut parser :G, mut fail_parser: H) -> impl FnMut(&'a str) -> IResult<&'a str, (Option<ICreole<'a>>, Option<ICreole<'a>>)>
where F: FnMut(&'a str) -> IResult<&'a str, &'a str>,
      G: FnMut(&'a str) -> IResult<&'a str, ICreole<'a>>,
...
fn take_while_parser_fail_or_peek_tag<'a, F, G>(term_tag: &'static str, parser :F, fail_parser: G) -> impl FnMut(&'a str) -> IResult<&'a str, (Option<ICreole<'a>>, Option<ICreole<'a>>)>
where F: FnMut(&'a str) -> IResult<&'a str, ICreole<'a>> + Copy,
...
fn heading(input: &str) -> IResult<&str, ICreole> {
  map(separated_pair(
    ...
    |(level, body)| ICreole::Heading(level as u8, body)) (input)
}
fn dont_format(input: &str) -> IResult<&str, ICreole> {
  map(delimited(tag("{{{\n"), take_until("\n}}}"), tag("\n}}}")), |body| ICreole::DontFormat(body))(input)
}

fn table_header_cell_inner(input: &str) -> IResult<&str, ICreole> {
  map(take_lit_text_until0("|"), |v| ICreole::TableHeaderCell(v))
    (input)
}
fn table_cell_inner(input: &str) -> IResult<&str, ICreole> {
  map(take_lit_text_until0("|"), |v| ICreole::TableCell(v))
    (input)
}
fn table_header_row(input: &str) -> IResult<&str, ICreole> {
  let (left, line) = preceded(tag("|="), map(alt((
      terminated(verify(is_not("\n"), |s: &str| s.ends_with("|")), tag("\n")),
      verify(rest, |s: &str| s.len() > 1 && s.ends_with("|")))), |s: &str| &s[..(s.len()-1)]))(input)?;
  // debug!("table_header_row line : {}", line);
  
  let (_, rst) = map(separated_list1(tag("|="), table_header_cell_inner), |v| ICreole::TableHeaderRow(v))(line)?;
  Ok((left, rst))
}
fn table_cell_row(input: &str) -> IResult<&str, ICreole> {
  ...
  let (_, rst) = map(separated_list1(tag("|"), table_cell_inner), |v| ICreole::TableRow(v))(line)?;
  Ok((left, rst))
}
...
pub fn creoles(input: &str) -> Vec<ICreole> {
  if let Ok((_, v)) = try_creoles(input) { v } else { vec![] }
}
...
mod tests {
  ...
  fn text_style_tests(){ init();
    ...
    assert_eq!(try_creoles("//i//"), Ok(("", vec![Line(vec![Italic("i")])])));

    assert_eq!(try_creoles("a**b**//c//d"), Ok(("", vec![Line(vec![Text("a"), Bold("b"), Italic("c"), Text("d")])])));
  }

  #[test]
  fn linebreak_tests(){ init();
    use ICreole::*;
    assert_eq!(try_creoles("a\\\\b"), Ok(("", vec![Line(vec![
      Text("a"), ForceLinebreak, Text("b"),
    ])])));
  ...
  fn list_tests() { init();
    ...
    assert_eq!(list("* a"), Ok(("", BulletList(vec![ListItem(vec![Text("a")])]))));
    assert_eq!(list("** b"), Ok(("", BulletList(vec![BulletList(vec![ListItem(vec![Text("b")])])]))));
    assert_eq!(list("*** c"), Ok(("", BulletList(vec![BulletList(vec![BulletList(vec![ListItem(vec![Text("c")])])])]))));
    assert_eq!(list("* a
** b"), Ok(("", BulletList(vec![
      ListItem(vec![Text("a")]),
      BulletList(vec![
...
** ab
* b
** ba"), Ok(("", BulletList(vec![
    ListItem(vec![Text("a")]),
    BulletList(vec![
      ListItem(vec![Text("ab")])
    ]),
    ListItem(vec![Text("b")]),
    BulletList(vec![
      ListItem(vec![Text("ba")])])]))));
    assert_eq!(list("* [[a]]
** //b//
** **c**"), Ok(("", BulletList(vec![
      ListItem(vec![Link("a", "a")]),
      BulletList(vec![
        ListItem(vec![Italic("b")]),
        ListItem(vec![Bold("c")]),
      ]),
    ]))));
    assert_eq!(list("* a
...
* b"), Ok(("", vec![
  ...
  #[test]
  fn parser_tests(){ init();
    use ICreole::*;
    assert_eq!(take_while_parser_fail(lit, text)("a b **a**"), Ok(("", (
      Some(Text("a b ")),
      Some(Bold("a")),
    ))));
  ...
  fn table_tests(){ init();
    use ICreole::*;
    assert_eq!(table_header_row("|=a|=|=c|"), Ok(("", TableHeaderRow(vec![
      TableHeaderCell(vec![Text("a")]),
      TableHeaderCell(vec![]),
      TableHeaderCell(vec![Text("c")]),
    ]))));
    assert_eq!(table("|=a|=|=c|"), Ok(("", Table(vec![
      TableHeaderRow(vec![
        TableHeaderCell(vec![Text("a")]),
        TableHeaderCell(vec![]),
        TableHeaderCell(vec![Text("c")]),
    ...
    assert_eq!(table("|=|=table|=header|
|a|table|row|
|b|table|row|
|c||empty|"), Ok(("", Table(vec![
      TableHeaderRow(vec![
        TableHeaderCell(vec![]),
    ...
    assert_eq!(try_creoles("|=|=a|=b|
|0|1|2|
|3|4|5|"), Ok(("", vec![Table(vec![
      TableHeaderRow(vec![
        TableHeaderCell(vec![]),
        TableHeaderCell(vec![Text("a")]),
  ...
  #[test]
  fn image_tests() { init(); use ICreole::*;
    ...
    assert_eq!(creoles("{{a.jpg|label}}"), vec![Line(vec![Image("a.jpg", "label")])]);

    assert_eq!(creoles("{{a.jpg|[[label]]}}"), vec![Line(vec![Image("a.jpg", "[[label]]")])]);
  }
  #[test]
  fn other_tests(){ init(); use ICreole::*;
